    ...
    def reload_config(self):
        with self._lock:
            try:
                ...
                print(f"[WARN] Failed to load config '{self.filepath}': {e}")
                self.config = self._default_config()
                try:
                    # Attempt to recreate the config file with defaults
                    import os

                    os.makedirs(os.path.dirname(self.filepath), exist_ok=True)
                    with open(self.filepath, "w") as f:
                        json.dump(self.config, f, indent=4)
                    print(f"[INFO] Created default config at '{self.filepath}'")
                except Exception as write_err:
                    raise RuntimeError(f"Failed to write default config: {write_err}")
