...
class Theme():
        ...
        try:
            match font_purpose:
                ...
                case FontPurpose.LIST | FontPurpose.DESCRIPTIVE_LIST_TITLE | FontPurpose.DESCRIPTIVE_LIST_DESCRIPTION:
                    list_cfg = cls._data.get("list") or {}
                    if list_cfg.get("color"):
                        return cls.hex_to_color(list_cfg.get("color"))
                    else:
                        return cls.hex_to_color(cls._data["grid"]["color"])
                case FontPurpose.MESSAGE:
                    return cls.hex_to_color(cls._data["grid"]["color"])
    ...
    def text_color_selected(cls, font_purpose : FontPurpose):
        try:
            match font_purpose:
                ...
                case FontPurpose.LIST | FontPurpose.DESCRIPTIVE_LIST_TITLE | FontPurpose.DESCRIPTIVE_LIST_DESCRIPTION:
                    list_cfg = cls._data.get("list") or {}
                    if list_cfg.get("selectedcolor"):
                        color = cls.hex_to_color(list_cfg.get("selectedcolor"))
    ...
    def get_cfw_default_icon(cls, icon_name):
        ...
        if(cfw_theme is None):
            ...
            if not theme_dir:
                PyUiLogger.get_logger().debug("themeDir is None/empty, cannot resolve CFW icon path")
                return None
