...
IMAGE_PATH="/mnt/SDCARD/icons/systemoptions/816.png"
...
update_cpu_config_name() {
    if [ -f "$config_path" ]; then
        ...
        sed -i 's|"name": "Set CPU to 816 MHz"|"name": "✓ CPU is at 816 MHz"|g' "$config_path"
    fi
}
...
if command -v sed &> /dev/null; then
    sed -i 's|\(/mnt/SDCARD/App/utils/utils performance [12] \)[0-9]*|\1816|g' "$script_path"
fi
