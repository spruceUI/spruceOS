...
IMAGE_PATH="/mnt/SDCARD/icons/systemoptions/1512.png"
...
update_cpu_config_name() {
    if [ -f "$config_path" ]; then
        ...
        sed -i 's|"name": "Set CPU to 1512 MHz"|"name": "✓ CPU is at 1512 MHz"|g' "$config_path"
    fi
}
...
if command -v sed &> /dev/null; then
    sed -i 's|\(/mnt/SDCARD/App/utils/utils performance [12] \)[0-9]*|\11512|g' "$script_path"
fi
