...
#pragma parameter palette "PALETTE" 0.0 0.0 4.0 1.0
...
void main()
{
   ...
   vec4 res = vec4(COMPAT_TEXTURE(Source, mod_texel / SourceSize.xy).rgb, 1.0);
   if (res.x > 0.85) res.rgb = vec3(0.7,0.7,0.7);
   else if (res.x > 0.6) res.rgb = vec3(0.45,0.45,0.45);
   else if (res.x > 0.3) res.rgb = vec3(0.2,0.2,0.2);
   else res.rgb = vec3(0.05,0.05,0.05);
   FragColor = res;
} 
#endif
