...
home_key_down () {
    ...
    if [ ! -e /tmp/menubtn ]; then
        ...
        if [ "$HOLD_HOME" = "Hotkeys" ]; then
            # MENU becomes a live chord modifier instead of a single
            # timed action, so there is no hold-timer to start here. The
            # A/B/L/R/Start chords are handled by RetroArch's own hotkey
            # remap (spruce/scripts/emu/lib/ra_functions.sh) while a game
            # is running under RetroArch; the vol_up/vol_down brightness
            # chord is handled by buttons_watchdog.sh. The existing
            # cancel_menu_hold case bucket below still fires for every one
            # of those button presses, which is what keeps the tap-home
            # action (see home_key_up) from also firing on release.
            menu_hold_pid=""
        else
...
# cancel_menu_hold() never fires for them and a chorded vol_up/vol_down
# still triggers the tap/hold-home action on MENU release. Mirrors the same
# conditional EVENT_PATH_VOLUME handling buttons_watchdog.sh already does.
