...
log_message "GeoSyncSystemTime: Attempting to set system time using NTP..."
...
if [ "$status" = "success" ]; then
    ...
    log_message "GeoSync successful, new timezone: $TZ new system time: $(date)"
    flag_add "time_synced_this_boot";
    exit 0
fi
exit 1