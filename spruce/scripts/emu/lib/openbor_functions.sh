...
run_openbor() {
	...
	if [ "$PLATFORM" = "Flip" ]; then
		...
		./OpenBOR_Flip "$ROM_FILE" > $(emu_log_file) 2>&1
		...
		if [ "$GAME" = "Final Fight LNS.pak" ]; then
			./OpenBOR_mod "$ROM_FILE" > $(emu_log_file) 2>&1
		else
			./OpenBOR_new "$ROM_FILE" > $(emu_log_file) 2>&1
		fi
		...
		./OpenBOR_TrimUI "$ROM_FILE" > $(emu_log_file) 2>&1
	fi
	sync
