...
EMULATORS="ra32.miyoo ra64.miyoo ra64.trimui_Brick ra64.trimui_SmartPro retroarch retroarch-flip drastic32 drastic64 PPSSPPSDL PPSSPPSDL_Flip PPSSPPSDL_Brick PPSSPPSDL_SmartPro MainUI flycast yabasanshiro yabasanshiro.trimui"
...
while true; do
    ...
    if flag_check "wake.alarm"; then
        ...
        if [ -z "$CURRENT_ALARM" ]; then
            ...
            if pgrep "MainUI" >/dev/null || pgrep "ra32.miyoo" >/dev/null || pgrep "ra64.miyoo" >/dev/null || pgrep "drastic*" >/dev/null || pgrep "PPSSPP*" >/dev/null || pgrep "flycast" >/dev/null || pgrep "yaba*" >/dev/null; then
                /mnt/SDCARD/spruce/scripts/save_poweroff.sh
            fi
